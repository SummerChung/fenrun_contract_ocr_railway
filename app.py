...
st.title("分潤增補協議書 OCR 自動辨識工具（最終穩定版）")
    ...
    if len(uploaded_files) > 10:
        ...
        for i, file in enumerate(uploaded_files):
            ...
            try:
                ...
                price_match = re.search(r"[單价價][：:NT\$ ]*([0-9,]{2,6})", full_text)
                total_match = re.search(r"[合总總]計[：:NT\$ ]*([0-9,]{2,8})", full_text)
                count_match = re.search(r"[數数量台]+[:： ]*([一二三四五六七八九十百千壹貳參肆伍陸柒捌玖拾佰仟]+)", full_text)
                commission_match = re.search(r"[分酬][潤利][金費額]?[：:新台幣 ]*([一二三四五六七八九十百千壹貳參肆伍陸柒捌玖拾佰仟]+)", full_text)
